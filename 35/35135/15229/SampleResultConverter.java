...
public class SampleResultConverter extends AbstractCollectionConverter
{
   ...
   {
      if (save.saveTime())
            writer.addAttribute("time", Long.toString(res.getTime()));
      if (save.saveLatency())
            writer.addAttribute("latency", Long.toString(res.getLatency()));
      if (save.saveTimestamp())
            writer.addAttribute("timeStamp", Long.toString(res.getTimeStamp()));
      if (save.saveSuccess())
            writer.addAttribute("success", //JDK1.4 Boolean.toString(res.isSuccessful()));
            		JOrphanUtils.booleanToString(res.isSuccessful()));
      if (save.saveLabel())
            writer.addAttribute("label", ConversionHelp.encode(res
                  .getSampleLabel()));
      if (save.saveCode())
            writer.addAttribute("responseCode", ConversionHelp.encode(res
                  .getResponseCode()));
      if (save.saveMessage())
            writer.addAttribute("responseMessage", ConversionHelp.encode(res
                  .getResponseMessage()));
      if (save.saveThreadName())
            writer.addAttribute("threadName", ConversionHelp
                  .encode(res.getThreadName()));
      if (save.saveDataType())
            writer.addAttribute("dataType", ConversionHelp.encode(res.getDataType()));
      if (save.saveEncoding())
            writer.addAttribute("dataEncoding", ConversionHelp.encode(res
                  .getDataEncoding()));
   }
   ...
   protected void retrieveAttributes(HierarchicalStreamReader reader,
         SampleResult res)
   {
      res.setSampleLabel(ConversionHelp.decode(reader.getAttribute("label")));
      res.setDataEncoding(ConversionHelp.decode(reader.getAttribute("dataEncoding")));
      res.setDataType(ConversionHelp.decode(reader.getAttribute("dataType")));
      res.setResponseCode(ConversionHelp.decode(reader.getAttribute("responseCode")));
      res.setResponseMessage(ConversionHelp.decode(reader.getAttribute("responseMessage")));
      res.setSuccessful(Converter.getBoolean(reader.getAttribute("success"), true));
      res.setThreadName(ConversionHelp.decode(reader.getAttribute("threadName")));
      res.setTime(Converter.getLong(reader.getAttribute("time")));
      res.setTimeStamp(Converter.getLong(reader.getAttribute("timeStamp")));
      res.setLatency(Converter.getLong(reader.getAttribute("latency")));
   }
