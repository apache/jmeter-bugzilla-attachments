...
// DJ removed   MultipartEntity multiPart = new MultipartEntity(
...
// DJ removed    StringBody stringBody = new StringBody(arg.getValue(), charset);
//  DJ removed   FormBodyPart formPart = new FormBodyPart(arg.getName(), stringBody);
//  DJ removed    multiPart.addPart(formPart);
//DJ added 	
               mep.addBinaryBody(arg.getName(), arg.getValue().getBytes(contentEncoding));
               mep.setLaxMode();
            }
...
//  DJ removed    multiPart.addPart(file.getParamName(),fileBodies[i]);
				 mep.addPart(file.getParamName(), fileBodies[i] );
            }
//DJ added 	
HttpEntity pst = mep.build();
            post.setEntity(pst);
//DJ Changed from multipart to pst
            if (pst.isRepeatable()){
